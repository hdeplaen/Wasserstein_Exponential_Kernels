% COMPUTE VALUES FOR QUICKDRAW
tol = 5e-2 ;

[trX, trY, teX, teY] = load_quickdraw(Ntr, Nte) ;
[trX,trY] = group_data(trX,trY) ;
[teX,teY] = group_data(teX,teY) ;
disp('Loading complete') ;

save('Y.mat','trY','teY','-v7.3') ;

N2 = ones(length(trY),1) ;
Nt2 = ones(length(teY),1) ;

NW = sqrt(squeeze(sum(sum(trX.^2,1),2))) ;
NtW = sqrt(squeeze(sum(sum(teX.^2,1),2))) ;
